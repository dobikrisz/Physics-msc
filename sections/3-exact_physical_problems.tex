...
\end{equation}
Most használjuk a szögösszeg képletet\footnote{
$sin(a + b) = sin(a) cos(b) + cos(a) sin(b)$ és $cos(a + b) = cos(a) cos(b) - sin(a) sin(b)$
}, hogy kifejezzük a $sin(\omega t + \phi)$ és $cos(\omega t + \phi)$-t:
\begin{equation}
    \begin{aligned}
        & \left( -A \omega^2 + \omega_0^2 A \right) \left( sin(\omega t) cos(\phi) + cos(\omega t) sin(\phi) \right) + \\
        & 2\beta A \omega \left( cos(\omega t) cos(\phi) - sin(\omega t) sin(\phi) \right) = \frac{F_0}{m} sin(\omega t)
    \end{aligned}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        & A^2 \left( \left( -\omega^2 + \omega_0^2 \right) cos(\phi) - 2\beta  \omega sin(\phi) \right)^2 + \\
        & A^2 \left( \left( - \omega^2 + \omega_0^2 \right) sin(\phi) + 2\beta  \omega cos(\phi) \right)^2 = \left(\frac{F_0}{m}\right)^2
    \end{aligned}
\end{equation}
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \frac{d}{dt} (\mathbf{v} \times \mathbf{K} - \gamma M \mathbf{\hat{e}}_r) = \vec{0}
\end{equation}
Vagyis:
\begin{equation}
    \mathbf{v} \times \mathbf{K} - \gamma M \mathbf{\hat{e}}_r = \mathbf{C} \qquad \mathbf{C} = \text{Áll.}
\end{equation}
Ezt nevezik a Laplace-Runge-Lenz vektornak. Rendezzük át egy kicsit:
\begin{equation}
    \begin{aligned}
        \mathbf{v} \times \mathbf{K} &= \mathbf{C} + \gamma M \mathbf{\hat{e}}_r \\
        \mathbf{v} \times \mathbf{K} &= GM (\boldsymbol{\varepsilon} +\mathbf{\hat{e}}_r) \qquad \boldsymbol{\varepsilon} = \frac{\mathbf{C}}{GM}
    \end{aligned}
\end{equation}
Ahol bevezzük az $\boldsymbol{\varepsilon}$ mennyiséget aminek a nagyságát ekszcentricitásnak neveznek. Ez a vektor mindig a periapszis felé mutat, amelyik az a pont a pályán ahol a test a legközelebb van a naphoz. Tehát a $\boldsymbol{\varepsilon}$ vektor iránya a pálya orientációját határozza meg, míg a nagysága az excentricitást, ami a pálya alakját jellemzi. Ha $\varepsilon = 0$, akkor a pálya kör alakú, ha $0 < \varepsilon < 1$, akkor elliptikus, ha $\varepsilon = 1$, akkor parabolikus, és ha $\varepsilon > 1$, akkor hiperbolikus.:
\begin{equation}
...
\end{equation}
\begin{equation}
    \hat{a}_- \hat{a}_+ = \frac{1}{2 \hbar m \omega} \left[\hat{p}^2 + (m \omega \hat{x})^2\right] + \frac{i}{2 \hbar} [\hat{x}, \hat{p}]
\end{equation}
Itt pedig észrevehetjük a Hamilton-operátort:
\begin{equation}
    \hat{a}_- \hat{a}_+ = \frac{1}{\hbar \omega} \hat{H} + \frac{i}{2 \hbar} [\hat{x}, \hat{p}]
\end{equation}
...
\begin{equation}
    \hat{a}_- \hat{a}_+ = \frac{1}{\hbar \omega} \hat{H} + \frac{1}{2} [\hat{x}, \hat{p}] = \frac{1}{\hbar \omega} \hat{H} + \frac{1}{2} i \hbar
\end{equation}
...
\begin{equation}
    \begin{aligned}
        & \varrho^{l - 1} e^{-\varrho} \left\{\left[-2l - 2 + \varrho + \frac{l(l+1)}{\varrho}\right]v + 2(l + 1 - \varrho) \frac{dv}{d \varrho} + \varrho \frac{d^2 v}{d \varrho^2}\right\} = \\
        & = \left[1 - \frac{\varrho_0}{\varrho} + \frac{l(l + 1)}{\varrho^2}\right] \varrho^{l + 1} e^{-\varrho} v
    \end{aligned}
\end{equation}
\begin{equation}
